...
router.get('/route', async (req, res) => {
    ...
    try {
        ...
        const comparison = (0, pricing_1.calculateFaresAndScores)(distanceKm, durationMins, sourceLabel, destLabel);
        // Calculate savings
...
router.post('/redirect', async (req, res) => {
    ...
    try {
        if (pool_1.isFallbackMode) {
            ...
            // PostgreSQL Active Path
            if (searchId) {
                await pool_1.pool.query('UPDATE searches SET selected_provider = $1 WHERE id = $2', [provider, searchId]);
            }
            const checkQuery = `
        SELECT id FROM analytics 
        WHERE provider = $1 AND created_at >= CURRENT_DATE
      `;
            const checkRes = await pool_1.pool.query(checkQuery, [provider]);
          ...
          WHERE id = $2
        `;
                await pool_1.pool.query(updateQuery, [fare || 0, checkRes.rows[0].id]);
            }
            else {
                const insertQuery = `
          ...
          VALUES ($1, 1, 1, $2)
        `;
                await pool_1.pool.query(insertQuery, [provider, fare || 0]);
            }
        }
...
router.get('/analytics', async (req, res) => {
    try {
        if (pool_1.isFallbackMode) {
            ...
            // PostgreSQL Active Path
            const totalSearchesRes = await pool_1.pool.query('SELECT COUNT(*)::int as count FROM searches');
            const totalSearches = totalSearchesRes.rows[0].count;
            const avgSavingsRes = await pool_1.pool.query('SELECT COALESCE(AVG(savings), 0)::float as avg_savings FROM searches');
            const avgSavings = Math.round(avgSavingsRes.rows[0].avg_savings);
            const popRoutesRes = await pool_1.pool.query(`
      ...
      `);
            ...
            const providerShareRes = await pool_1.pool.query(`
      ...
      `);
            ...
            const dailyTrendsRes = await pool_1.pool.query(`
        SELECT 
